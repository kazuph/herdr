...
### Herdr monorepo installation

```bash
just terminal-browser-install
```

Run this command from the Herdr repository root. The monorepo build uses pinned
upstream revisions and checksums, rejects high-severity production dependency
advisories, signs the macOS bundle with an available Developer ID identity, and
links the local plugin without downloading an executable at plugin-build time.

For the standalone upstream installation instructions, see
