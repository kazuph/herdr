...
export function agentBrowserConfigPath(): string {
  return DIST_ROOT
    ? path.join(DIST_ROOT, "agent-browser", "config.json")
    : path.resolve(__dirname, "..", "..", "scripts", "agent-browser.json");
}
...
export function agentBrowserEnvironment(socketDir = AGENT_SOCKETS_DIR): NodeJS.ProcessEnv {
