...
CHECKSUMS="$ROOT/scripts/electron-sha256.txt"

expected="$(awk -v zip="*$ZIP" '$2 == zip {print $1}' "$CHECKSUMS")"
if [ -z "$expected" ]; then
  echo "fetch-electron: $ZIP is missing from $CHECKSUMS" >&2
  exit 1
fi
...
if [ "$actual" != "$expected" ]; then
  echo "fetch-electron: $ZIP does not match the pinned repository checksum" >&2
