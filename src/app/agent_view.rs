//! Plugin-driven projection over the built-in Agents view.
//!
//! Display only (UP-AUTOMATION-P4): an active view filters and sorts the
//! sidebar agent entries. Startup hooks stay rejected per UP-AUTOMATION
//! policy; this module never executes plugin commands.
...
fn builtin_field_value(
    ...
    match field {
        AgentViewBuiltinField::Status => Some(EvalValue::String(
            agent_panel_status_key(entry.state, entry.seen).to_string(),
        )),
...
mod tests {
    ...
    fn source_and_label_normalize() {
        ...
        assert_eq!(spec.label.as_deref(), Some("current"));

        let mut bad = AgentViewSetParams {
            source: "has space".to_string(),
            label: None,
            filter: None,
            sort: Vec::new(),
        };
        assert!(validate_agent_view(&mut bad).is_err());
    }
    ...
    fn unknown_status_value_is_rejected() {
        let mut spec = AgentViewSetParams {
            ...
            filter: Some(AgentViewFilter::Eq {
                ...
                value: AgentViewValue::String("napping".to_string()),
            }),
            sort: Vec::new(),
        };
