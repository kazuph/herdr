...
impl App {
    pub(crate) fn handle_copy_mode_key(&mut self, key: TerminalKey) {
        ...
        self.state.handle_copy_mode_key(key);
        if let Some(request) = self.state.request_clipboard_write.take() {
                ...
                .is_ok()
            {
                ...
                self.selection_copy_status_deadline =
                    Some(std::time::Instant::now() + super::SELECTION_COPY_STATUS_DURATION);
            } else {
...
impl AppState {
    ...
    pub(crate) fn handle_copy_mode_key(&mut self, key: TerminalKey) {
        match key.code {
            ...
            KeyCode::PageUp => {
                self.scroll_copy_mode_page(-1, false);
                return;
            }
            KeyCode::PageDown => {
                self.scroll_copy_mode_page(1, false);
            ...
            KeyCode::Home => {
                self.copy_mode_line_edge(false);
                return;
            }
            KeyCode::End => {
                self.copy_mode_line_edge(true);
                return;
            }
            _ => {}
        }
        ...
        match (key.code, key.modifiers) {
            (KeyCode::Char('u'), mods) if mods.contains(KeyModifiers::CONTROL) => {
                self.scroll_copy_mode_page(-1, true)
            }
        ...
        match ch {
            'q' => self.exit_copy_mode(false),
            'y' => self.exit_copy_mode(true),
            'v' | ' ' => self.begin_copy_mode_selection(),
            'V' => self.select_copy_mode_line(),
            'h' => self.move_copy_cursor(0, -1),
            'j' => self.move_copy_cursor(1, 0),
            'k' => self.move_copy_cursor(-1, 0),
            'l' => self.move_copy_cursor(0, 1),
            'g' => self.copy_mode_history_top(),
            'G' => self.copy_mode_history_bottom(),
            '0' => self.copy_mode_line_edge(false),
            '$' => self.copy_mode_line_edge(true),
            '^' => self.copy_mode_first_non_blank(),
            'w' => self.copy_mode_word_motion(WordMotion::NextStart),
            'b' => self.copy_mode_word_motion(WordMotion::PreviousStart),
            'e' => self.copy_mode_word_motion(WordMotion::NextEnd),
            '{' => self.copy_mode_paragraph(-1),
            '}' => self.copy_mode_paragraph(1),
            _ => {}
        }
    }
    ...
    fn begin_copy_mode_selection(&mut self) {
        ...
        let metrics = self.pane_scroll_metrics(copy_mode.pane_id);
    ...
    fn select_copy_mode_line(&mut self) {
    ...
    fn move_copy_cursor(&mut self, row_delta: i16, col_delta: i16) {
        let Some(mut copy_mode) = self.copy_mode else {
            return;
        };
        let Some(info) = self.pane_info_by_id(copy_mode.pane_id).cloned() else {
        ...
        if row_delta < 0 {
            ...
            if copy_mode.cursor_row >= delta {
                ...
                self.scroll_pane_up(copy_mode.pane_id, usize::from(delta));
            ...
            if copy_mode.cursor_row.saturating_add(delta) <= bottom {
                ...
                self.scroll_pane_down(copy_mode.pane_id, usize::from(delta));
    ...
    fn scroll_copy_mode_page(&mut self, direction: i16, half_page: bool) {
        let Some(mut copy_mode) = self.copy_mode else {
            return;
        };
        let Some(info) = self.pane_info_by_id(copy_mode.pane_id).cloned() else {
        ...
        if let Some(metrics) = self.pane_scroll_metrics(copy_mode.pane_id) {
            if direction < 0 {
                ...
                if next_offset > metrics.max_offset_from_bottom {
                    ...
                    self.set_pane_scroll_offset(copy_mode.pane_id, metrics.max_offset_from_bottom);
                    copy_mode.cursor_row = copy_mode
                        .cursor_row
                        .saturating_sub(cursor_lines.min(u16::MAX as usize) as u16);
                } else {
                    self.set_pane_scroll_offset(copy_mode.pane_id, next_offset);
                }
            } else if metrics.offset_from_bottom < lines {
                let cursor_lines = lines.saturating_sub(metrics.offset_from_bottom);
                self.set_pane_scroll_offset(copy_mode.pane_id, 0);
                copy_mode.cursor_row = copy_mode
                    .cursor_row
                    .saturating_add(cursor_lines.min(u16::MAX as usize) as u16)
                    .min(info.inner_rect.height.saturating_sub(1));
            } else {
                self.set_pane_scroll_offset(copy_mode.pane_id, metrics.offset_from_bottom - lines);
            }
        } else if direction < 0 {
            self.scroll_pane_up(copy_mode.pane_id, lines);
        } else {
            self.scroll_pane_down(copy_mode.pane_id, lines);
        }
        self.copy_mode = Some(copy_mode);
        self.sync_copy_mode_selection();
    }

    fn copy_mode_history_top(&mut self) {
        let Some(mut copy_mode) = self.copy_mode else {
            return;
        };
        let Some(metrics) = self.pane_scroll_metrics(copy_mode.pane_id) else {
            return;
        };
        self.set_pane_scroll_offset(copy_mode.pane_id, metrics.max_offset_from_bottom);
        copy_mode.cursor_row = 0;
    ...
    fn copy_mode_history_bottom(&mut self) {
        let Some(mut copy_mode) = self.copy_mode else {
            return;
        };
        let Some(info) = self.pane_info_by_id(copy_mode.pane_id) else {
    ...
    fn copy_mode_line_edge(&mut self, end: bool) {
        let Some(mut copy_mode) = self.copy_mode else {
            return;
        };
        let Some(info) = self.pane_info_by_id(copy_mode.pane_id) else {
    ...
    fn copy_mode_first_non_blank(&mut self) {
        ...
        let Some(text) = self.copy_mode_visible_row_text(copy_mode.cursor_row) else {
            return;
        };
    ...
    fn copy_mode_word_motion(&mut self, motion: WordMotion) {
        let Some(mut copy_mode) = self.copy_mode else {
            return;
        };
        let Some(info) = self.pane_info_by_id(copy_mode.pane_id) else {
    ...
    fn copy_mode_paragraph(&mut self, direction: i16) {
        let Some(copy_mode) = self.copy_mode else {
            return;
        };
        ...
        let limit = self
            .pane_scroll_metrics(copy_mode.pane_id)
        ...
        for _ in 0..limit {
            ...
            self.move_copy_cursor(direction, 0);

            let Some(after) = self.copy_mode else {
                return;
            };
            if self
                .copy_mode_visible_row_text(after.cursor_row)
                .is_some_and(|text| text.trim().is_empty())
            {
                return;
            }

            let Some(after_metrics) = self.pane_scroll_metrics(after.pane_id) else {
                continue;
            };
            let did_not_move =
    ...
    fn copy_mode_visible_row_text(&self, viewport_row: u16) -> Option<String> {
        ...
        self.runtime_for_pane_in_workspace(ws_idx, copy_mode.pane_id)?
    ...
    fn sync_copy_mode_selection(&mut self) {
        ...
        match selection {
            CopyModeSelection::Character => {
                ...
                self.update_selection_cursor(copy_mode.pane_id, screen_col, screen_row);
            }
...
mod tests {
    ...
    async fn copy_mode_ctrl_u_moves_cursor_when_history_top_clamps() {
        ...
        app.state
            .set_pane_scroll_offset(pane_id, metrics.max_offset_from_bottom - lines + 1);
        if let Some(copy_mode) = app.state.copy_mode.as_mut() {
            copy_mode.cursor_row = bottom;
        }
    ...
    async fn copy_mode_ctrl_d_moves_cursor_when_live_bottom_clamps() {
        ...
        let lines = copy_mode_page_lines(bottom + 1, true);
        assert!(lines > 1);
        app.state.set_pane_scroll_offset(pane_id, lines - 1);
        if let Some(copy_mode) = app.state.copy_mode.as_mut() {
            copy_mode.cursor_row = 0;
        }
    ...
    async fn copy_mode_q_restores_entry_scrollback_offset() {
        ...
        let (mut app, pane_id) = app_with_copy_scrollback(&bytes);
        let entry_offset = 3;
        app.state.set_pane_scroll_offset(pane_id, entry_offset);
