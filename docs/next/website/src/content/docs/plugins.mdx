...
`placement = "popup"` opens a terminal popup in the current workspace without
changing the tiled layout. It accepts optional `width` and `height` fields in the manifest or
open request; omit them for the default 80% popup, use numbers for outer
terminal-cell dimensions, or use strings like `"80%"` for a percentage of the
terminal area. Herdr keybindings can switch away while the popup keeps running;
returning to its workspace restores it. Mouse drag selection copies text. Escape
closes a plain popup, while interactive apps that enable terminal modes receive it.
It closes when the command exits
or a `popup.close` request is sent. Dimensions smaller than the popup minimum are clamped.
...
workspaces. A popup is a per-workspace resource rather than a Herdr pane: each
workspace may hold at most one popup, opening one in a second workspace does
not require closing the first, and only the active workspace's popup is
visible and receives input. A popup has no pane ID, does not change plugin
focus context, emits no pane lifecycle events, and does not participate in
pane, layout, persistence, or agent APIs.
