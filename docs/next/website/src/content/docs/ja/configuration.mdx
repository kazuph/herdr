...
コンテキストメニューの `New Claude Code agent`、`New Codex agent`、`New agy agent` は、設定がある場合にその argv を使います。既存の起動フラグを、シェルの alias や既存 pane を変更せずに引き継げます:
...
コマンドを省略した Agent は、追加引数なしで対応する実行ファイルを起動します。設定のリロードは以後のコンテキストメニュー起動に反映され、既存の Agent pane は再起動しません。

Codexのsession restoreも、設定された`agent_start.commands.codex`のargvを再利用し、末尾へ`resume <session-id>`だけを追加します。approval、sandbox、network、wrapperの各オプションをこの構造化argvへ置くと、新規paneと復元paneが同じ条件でCodexを起動します。このargvが無い場合は、従来の`agent_restore.commands.codex` templateを使います。
...
`type = "popup"` は、タブレイアウトを変更せずに現在のワークスペースへポップアップを開きます。
Herdr の直接キーバインドとプレフィックスキーバインドは引き続き優先されます。別のワークスペースへ移動するとコマンドを実行したままポップアップが隠れ、起動元へ戻ると再表示されます。
マウスのドラッグ選択は通常のペインと同じようにテキストをコピーします。通常のポップアップは Escape で閉じ、ターミナルモードを有効にした対話型アプリには Escape を渡します。それ以外の入力は、コマンドが終了するまでポップアップへ送られます。
`width` と `height` は任意です。省略するとターミナル領域の80%になり、数値ならターミナルセル数、`"80%"` のような文字列ならターミナル領域に対する割合を指定できます。
