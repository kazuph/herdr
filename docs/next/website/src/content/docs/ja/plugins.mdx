...
`placement = "popup"` は、タイルレイアウトを変更せずに現在のワークスペースへターミナルポップアップを開きます。
マニフェストまたは open リクエストで任意の `width` と `height` を指定できます。
省略するとターミナル領域の80%になり、数値なら外側のターミナルセル数、`"80%"` のような文字列ならターミナル領域に対する割合になります。
Herdr のキーバインドで別のワークスペースへ移動してもポップアップは実行を続け、起動元へ戻ると再表示されます。マウスのドラッグ選択はテキストをコピーします。通常のポップアップは Escape で閉じ、ターミナルモードを有効にした対話型アプリには Escape を渡します。コマンドが終了するか `popup.close` リクエストが送られると閉じます。
...
ポップアップは Herdr ペインではなく、Space ごとに 1 つだけ存在できるリソースです。
別の Space では先に開いたポップアップを閉じずに新しいポップアップを開けます。表示と入力の対象になるのは、現在の Space のポップアップだけです。
