...
`pane current` は最初に `HERDR_PANE_ID` を実行中 server で検証します。
この環境変数がない場合、server は呼び出し元の process session、続いてその親
process tree から所有 pane を1件だけ解決します。所有 pane がない、または複数
候補がある時は、focused pane、active tab、pane list の順序へ fallback せず失敗します。
...
信頼済みローカルツールからエージェント状態、表示metadata、exact restore用sessionを報告する:
...
`agent read` は解決されたターミナルストリームを読みます。`agent send` は fork の従来のテキストと Enter による送信を維持します。`agent get`、`agent focus`、`agent wait`、`agent attach` は、解決されたターミナルがエージェントのアイデンティティを持つことを要求します。`agent rename` はそのアイデンティティを割り当てられます。

`agent prompt` は本家 0.9.1 の `agent.prompt` API を使います。対象には一意なエージェント名またはペイン ID（`p_N`、`pN`、`%N`）を指定できます。エージェントラベルとターミナル ID は prompt の対象には使えません。空文字は `empty_agent_prompt`、承認や質問への回答待ちは `agent_blocked`、既知のエージェントがいない・起動中・前面プロセスがそのエージェントではない場合は `agent_not_ready` を返します。これらの拒否では本文も Enter も送りません。

本文の末尾の改行を保持し、端末が bracketed paste を有効にしていれば、その形式で送信します。本文を書き終えてから 300 ms 待ち、Enter の書き込みに成功して初めて、エージェント情報を含む `agent_prompted` を返します。Copilot には本文より先にフォーカス取得を通知します。送信中に端末が閉じた場合は失敗を返します。

`--wait` は標準で `idle`、`done`、`blocked` のいずれかを待ちます。`--until STATUS` を繰り返すと対象の状態（`idle`、`working`、`done`、`blocked`、`unknown`）を選べ、`--timeout MS` で待機時間を制限できます。どちらも `--wait` が必要です。送信時に作業中でなかった相手では、まず `working` または `blocked` への遷移を確認します。メタデータやセッションの変更、以前から続く idle/done 状態では待機を完了しません。5秒以内に活動を確認できなければ `agent_prompt_stalled`、それより短い指定時間を超えれば `timeout`、対象のエージェントが失われれば `agent_not_running` を返します。fork の `agent send`、永続メールボックスの `herdr send <to> <text> --room <room>`、メールボックス入力も引き続き利用できます。

`agent start` の argv が `sh -lc`、`env`、言語ランタイムなどを経由しても、検出されたエージェントの準備完了を待ちます。検出された状態と、起動時の3秒の安定待ち・30秒の起動期限から、本家と同じ状態遷移で準備状態を判定します。保存時には起動途中の管理名を除外し、準備済みの管理情報を復元します。エージェントを検出しない通常の名前付きジョブは、fork の既存の名前を保持します。
...
この fork では `herdr integration` による agent hook/plugin の install/uninstall は行いません。信頼済みローカルツールから `pane report-agent` または `pane.report_agent` socket method で状態を報告してください。
...
`plugin pane open` は、プラグインがリンクされ、有効で、現在のプラットフォームと互換であることを要求します。マニフェストで宣言された `[[panes]]` コマンドを Herdr 管理のターミナルペインとして起動します。マニフェストのデフォルトは `overlay` で、アクティブなペインの上に一時的なズームオーバーレイを開きます。分割、新しいタブ、ズームされたペイン、またはタブレイアウトを変更しないワークスペース単位の `popup` として開くこともできます。ポップアップを実行したまま Herdr のキーバインドで別のワークスペースへ移動できます。`--width` と `--height` は、外側のポップアップ寸法をターミナルセル数または `80%` のような割合で指定します。省略した寸法はターミナル領域の80%になり、小さすぎる値はポップアップの最小サイズに制限されます。ポップアップは Herdr ペインではなく、`HERDR_PANE_ID` を受け取らず、pane API やエージェント API に参加しません。ターミナル以外のネイティブなプラグインペインは今後の対応面です。
