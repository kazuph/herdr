...
`agent read` は解決されたターミナルストリームを読みます。`agent send` は解決されたAI CLIへテキストを書き込み、入力欄に残らないようEnterで送信します。`agent get`、`agent focus`、`agent wait`、`agent attach` は、解決されたターミナルがエージェントのアイデンティティを持つことを要求します。`agent rename` はそのアイデンティティを割り当てられます。
