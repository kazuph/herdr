...
When your local and remote platforms match, Herdr can copy the current local binary for direct installs. This fork does not fetch release binaries from the upstream `latest.json` manifest. For package-manager installs, custom binaries, or platform mismatches, set `HERDR_REMOTE_BINARY` to an explicit local file path before running remote attach.
