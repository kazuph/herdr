---
title: Agents
description: See what Herdr can detect, how agent state works, custom labels, and direct attach.
---
...
Unsupported agents still run normally as terminal processes. They just may not get rich state unless a tool reports state over the socket API.

## How detection works

Herdr combines two built-in signals:

1. foreground process detection
2. terminal output heuristics

Process detection tells Herdr which pane owns an agent. Heuristics infer state without installing hooks or plugins into agent config directories.
...
Tools can report a visual status label without changing semantic state.
...
You can also start common agents from the UI. Right-click a workspace or pane
and choose Claude Code, Codex, or Gemini; Herdr creates a new split pane and
marks it as an agent target immediately.
