...
Herdr shows first-run setup when `onboarding` is missing or true. Continuing from onboarding writes `onboarding = false`. Set it when you want to skip that flow after setup.
...
`copy_mode` opens a Vim-style scrollback view for the focused pane. Use
`h`/`j`/`k`/`l` to move, `v` or `V` to select, and `y` or Enter to copy the
...
remove_worktree = "prefix+shift+delete"
...
Workspace context menus can create a new Git worktree, open an existing checkout,
or remove the selected linked checkout. New worktrees are created under:
...
`new_worktree` defaults to `prefix+shift+g`. `open_worktree` and
`remove_worktree` are unset by default so you can choose direct keyboard access
...
`sidebar_min_width` and `sidebar_max_width` control the expanded sidebar's resize bounds in columns. The defaults are 18 and 72.

`agent_panel_scope` can be `all` or `sort`. Use `sort` to group agents that need confirmation first, then running agents, then confirmed idle agents.

`workspace_panel_density` can be `full` or `slim`. Use `slim` to keep workspace cards on one line.

The default pane split bindings include tmux-style `prefix+percent` for side-by-side splits and `prefix+double_quote` for stacked splits. The older `prefix+v` and `prefix+minus` bindings remain available too.

`confirm_close` controls whether closing a workspace asks for confirmation. `prompt_new_tab_name` controls whether new tabs ask for a label first. Set `show_tab_bar = false` to hide the main content tab bar.

Set `vim_mode = true` to start terminal input in Vim Normal mode. Normal mode uses `j`/`k` for spaces, `h`/`l` for panes, `i` or `Enter` for Insert mode, and `Ctrl+[`/`Ctrl+]` for pane focus history. In Insert mode, keys are forwarded to the pane except `Ctrl+[`/`Ctrl+]`, which return to Normal mode.
...
Logs are useful when diagnosing startup warnings or socket API behavior.
