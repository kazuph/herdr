...
Mouse drag selection copies text like a tiled pane. All other terminal input, including
Escape, goes to the popup until its command exits. `width` and `height` are optional; omit them for the default half-size
