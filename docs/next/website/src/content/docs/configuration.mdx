...
Herdr shows first-run setup when `onboarding` is missing or true. Continuing writes `onboarding = false` and returns to the normal workspace view. Set it when you want to skip that flow after setup.
...
The `New Claude Code agent`, `New Codex agent`, and `New agy agent` context-menu actions use the configured argv when present. This preserves existing launch flags without changing shell aliases or existing panes:
...
When a command is omitted, Herdr starts the matching executable with no extra arguments. Reloading config changes subsequent context-menu launches; it does not restart existing agent panes.

Codex session restore also reuses the configured `agent_start.commands.codex` argv and appends only `resume <session-id>`. Put approval, sandbox, network, and wrapper options in this structured argv so new panes and restored panes start Codex under the same conditions. When this argv is absent, Herdr retains the legacy `agent_restore.commands.codex` template behavior.
...
`type = "popup"` opens a popup in the current workspace without changing the tab layout.
Herdr direct and prefix keybindings still take precedence, so switching workspaces hides
the popup while its command keeps running and returning to its workspace restores it.
Mouse drag selection copies text like a tiled pane. Escape closes a plain popup;
interactive apps that enable terminal modes receive Escape instead. Other terminal input goes to the popup until its command exits.
`width` and `height` are optional; omit them for the default 80% size
...
Only panes with a valid pane-specific native session reference can resume; other panes restore as normal shells. See [Session state and restore](/docs/session-state/) for supported Agents and persistence behavior.
