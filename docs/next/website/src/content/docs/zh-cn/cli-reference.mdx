...
`pane current` 首先通过运行中的服务器验证 `HERDR_PANE_ID`。没有该环境变量时，
服务器依次从调用进程的 process session 和父 process tree 中解析唯一的所属窗格。
如果没有所属窗格或存在多个候选，它会失败，不会回退到 focused pane、active tab
或 pane list 顺序。
...
`agent read` 读取解析出的终端流。`agent send` 保留 fork 原有的文本加 Enter 发送方式。`agent get`、`agent focus`、`agent wait` 和 `agent attach` 要求解析出的终端具有智能体身份。`agent rename` 可以赋予这个身份。

`agent prompt` 使用上游 0.9.1 的 `agent.prompt` API。目标可以是唯一的智能体名称或窗格 ID（支持 `p_N`、`pN` 和 `%N`）；智能体标签和终端 ID 不能作为 prompt 的目标。空文本返回 `empty_agent_prompt`；正在等待批准或回答的智能体返回 `agent_blocked`；没有已知智能体、启动尚未完成或前台进程已不再是该智能体时返回 `agent_not_ready`。这些拒绝不会发送文本或 Enter。

文本保留末尾换行，并在终端启用 bracketed paste 时使用该格式。Herdr 写完文本后等待 300 ms，再写入 Enter，只有写入成功后才返回包含智能体信息的 `agent_prompted`。发送给 Copilot 时先通知其获得焦点。发送期间终端关闭会导致提交失败。

`--wait` 默认等待 `idle`、`done` 或 `blocked`。可重复使用 `--until STATUS` 选择状态（`idle`、`working`、`done`、`blocked`、`unknown`），用 `--timeout MS` 限制等待时间；这两个选项都要求 `--wait`。向非工作状态的智能体发送时，必须先观察到 `working` 或 `blocked` 活动；元数据变化、会话变化以及原有的 idle/done 状态不能完成等待。5 秒内没有活动则返回 `agent_prompt_stalled`，更短的调用方期限到期则返回 `timeout`，目标智能体消失则返回 `agent_not_running`。fork 的 `agent send`、持久邮箱 `herdr send <to> <text> --room <room>` 和邮箱输入保持可用。

即使 `agent start` 的 argv 通过 `sh -lc`、`env` 或语言运行时启动，仍会根据检测到的状态以及 3 秒的稳定等待期和 30 秒的启动期限，按照上游的状态转换判断智能体是否就绪。快照不保存尚在启动中的托管名称，并恢复已就绪的托管信息。始终未检测到智能体的普通命名任务保留 fork 原有的名称。
...
这个 fork 不通过 `herdr integration` 安装或卸载智能体 hook/plugin。请让可信本地工具通过 `pane report-agent` 或 `pane.report_agent` socket method 报告状态。
...
`plugin pane open` 要求插件已链接、已启用并与当前平台兼容。它把清单声明的 `[[panes]]` 命令作为 Herdr 管理的终端窗格启动。清单的默认值是 `overlay`,在活动窗格上方打开一个临时的缩放覆盖层。它也可以作为分割、新标签页、缩放窗格,或不改变标签页布局的工作区级 `popup` 打开。弹窗运行时仍可使用 Herdr 快捷键切换工作区。`--width` 和 `--height` 以终端单元格数或 `80%` 这样的百分比设置弹窗外层尺寸;省略时默认为终端区域的80%,过小的值会限制为弹窗最小尺寸。弹窗不是 Herdr 窗格,不会收到 `HERDR_PANE_ID`,也不参与 pane 或智能体 API。非终端的原生插件窗格是之后的能力面。
