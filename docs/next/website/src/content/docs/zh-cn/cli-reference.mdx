...
`agent read` 读取解析出的终端流。`agent send` 向解析出的 AI CLI 写入文本并按 Enter 提交，避免消息停留在输入框中。`agent get`、`agent focus`、`agent wait` 和 `agent attach` 要求解析出的终端具有智能体身份。`agent rename` 可以赋予这个身份。
