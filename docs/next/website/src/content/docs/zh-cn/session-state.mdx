...
这个 fork 不会修改智能体配置目录。可信本地工具可以用 `herdr pane report-agent` 或 `pane.report_agent` socket method 报告准确的窗格状态和 session id。
...
实时交接用于需要替换运行中 Herdr 服务器的远程连接流程。它请求旧服务器把实时窗格转移给新服务器,让窗格进程跨服务器替换继续运行。
...
普通的 `herdr --remote workbox` 默认使用常规的重启/停止流程。这个 fork 禁用 self-update,因此 update handoff 不是受支持的安装路径。
