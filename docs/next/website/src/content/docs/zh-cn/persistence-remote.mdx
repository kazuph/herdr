...
当本地和远程平台一致时,对于直接安装,Herdr 可以直接复制当前本地二进制文件。这个 fork 不会从 `https://herdr.dev/latest.json` 下载发布产物。对于包管理器安装、自定义二进制文件或平台不一致的情况,请在远程连接前把 `HERDR_REMOTE_BINARY` 设置为明确的本地文件路径。
