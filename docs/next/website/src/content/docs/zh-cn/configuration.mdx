...
`type = "popup"` 会在当前工作区打开弹窗,而不改变标签页布局。
Herdr 的直接快捷键和前缀快捷键仍然优先。切换到其他工作区时,弹窗会隐藏但命令继续运行;返回原工作区时会再次显示。
鼠标拖动选择会像普通窗格一样复制文本。普通弹窗按 Escape 关闭;启用终端模式的交互式应用仍会收到 Escape。其他终端输入会发送到弹窗,直到命令退出。
`width` 和 `height` 是可选的;省略时默认为终端区域的80%,数字表示终端单元格数,`"80%"` 这样的字符串表示终端区域的百分比。
...
Codex 会话恢复会复用 `agent_start.commands.codex` 中配置的 argv，并且只在末尾追加 `resume <session-id>`。将 approval、sandbox、network 和 wrapper 选项放入这个结构化 argv，可让新窗格与恢复窗格以相同条件启动 Codex。未配置该 argv 时，Herdr 会保留旧的 `agent_restore.commands.codex` 模板行为。
