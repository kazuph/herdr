...
`placement = "popup"` 会在当前工作区打开终端弹窗,而不改变平铺布局。
可以在清单或 open 请求中指定可选的 `width` 和 `height`;省略时默认为终端区域的80%,数字表示外层终端单元格数,`"80%"` 这样的字符串表示终端区域的百分比。
使用 Herdr 快捷键切换到其他工作区时,弹窗会继续运行;返回原工作区时会再次显示。鼠标拖动选择会复制文本。普通弹窗按 Escape 关闭;启用终端模式的交互式应用仍会收到 Escape。命令退出或发送 `popup.close` 请求时弹窗会关闭。
小于弹窗最小尺寸的值会限制为最小值。
...
弹窗不是 Herdr 窗格,而是每个工作区最多一个的资源。在另一个工作区打开新弹窗时,无需关闭先前的弹窗;只有当前工作区的弹窗可见并接收输入。弹窗没有窗格 id,不会改变插件焦点上下文,不会发出窗格生命周期事件,也不参与 pane、layout、持久化或智能体 API。
