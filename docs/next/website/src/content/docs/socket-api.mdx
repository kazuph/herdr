...
- report custom agent state from trusted local tools
...
| Area | Methods |
| --- | --- |
| Server | `ping`, `server.stop`, `server.restart`, `server.reload_config`, `server.agent_manifests`, `server.reload_agent_manifests` |
...
context. Popup terminals belong to the workspace where they were opened and do
not change the tab layout. Herdr keybindings can switch away while the popup
keeps running, and returning to its workspace restores it;
optional `width` and `height` fields set their outer size as terminal cells or
percentages such as `"80%"`. Omitted dimensions default to 80% of the terminal
...
leaves plugin focus context on the underlying tiled pane, supports mouse drag
selection, and does not export
...
Trusted local tools report agent state with `pane.report_agent`.
...
Trusted local tools can report native session references with `pane.report_agent_session`, or include them in `pane.report_agent`. State-independent session reports do not affect waits, notifications, or rollups.
...
Use `pane.report_metadata` when a trusted local tool wants to customize presentation without taking over lifecycle state.
