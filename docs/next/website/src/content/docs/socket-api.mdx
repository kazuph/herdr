...
Pane commands accept short pane targets like `1-2`, global pane numbers like `23`, tmux-style global pane numbers like `%23`, and full stable IDs like `w6525bc3670d718-2`. The sidebar shows workspace numbers and agent pane IDs, and `herdr pane list` includes both the short ID and global ID for each pane.
...
| Pane | `pane.split`, `pane.list`, `pane.get`, `pane.focus`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.report_agent`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
...
Tools report agent state with `pane.report_agent`.
