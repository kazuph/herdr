---
title: CLI reference
description: Herdr commands for sessions, workspaces, tabs, panes, notifications, agents, waits, and status.
---

Herdr’s CLI talks to the running server over the same local socket API used by trusted local tools and agents.
...
`pane current` validates `HERDR_PANE_ID` with the running server first. Without
that variable, the server resolves one owning pane from the calling process
session, then its parent process tree. It fails instead of falling back to the
focused pane, active tab, or pane-list order when ownership is missing or
ambiguous.
...
Report agent state, display metadata, and an exact restore session from a trusted local tool:
...
`pane get`, `pane list`, `agent get`, and `agent list` include a read-only `agent_session` object when a safe native session reference is stored. If no native session reference is stored, the field is omitted.
...
`agent read` reads the resolved terminal stream. `agent send` retains the fork's existing text-and-Enter delivery. `agent get`, `agent focus`, `agent wait`, and `agent attach` require the resolved terminal to have agent identity. `agent rename` can assign that identity.

`agent prompt` uses the upstream 0.9.1 `agent.prompt` API. Its target is a unique agent name or a pane ID (`p_N`, `pN`, and `%N` are accepted); agent labels and terminal IDs are not prompt targets. An empty prompt returns `empty_agent_prompt`. A blocked agent returns `agent_blocked`; an unknown agent, a pending launch, or a pane whose foreground process is no longer that agent returns `agent_not_ready`. These rejections send neither text nor Enter.

Prompt text preserves trailing newlines and uses bracketed paste when the terminal enables it. After the text has been written, Herdr waits 300 ms, writes Enter, and only then returns `agent_prompted` with the agent information. Copilot receives focus gained before the text. Queued submissions fail if the terminal closes.

`--wait` waits for `idle`, `done`, or `blocked` by default. Repeat `--until STATUS` to select statuses (`idle`, `working`, `done`, `blocked`, or `unknown`); `--timeout MS` limits the wait. Both options require `--wait`. A prompt sent to a non-working agent must first produce observed `working` or `blocked` activity: metadata, session changes, and stale idle/done states do not satisfy this requirement. No activity within 5 seconds returns `agent_prompt_stalled`; an earlier caller deadline returns `timeout`. Loss of the target agent returns `agent_not_running`. The fork's `agent send`, durable mailbox `herdr send <to> <text> --room <room>`, and mailbox input remain available.

`agent start` also waits for readiness when its argv runs through a wrapper such as `sh -lc`, `env`, or a language runtime. Readiness follows upstream's detected-agent state transitions, with a 3-second settling period and a 30-second startup deadline. Snapshots omit pending managed names and preserve active managed ownership. Named jobs that never detect an agent keep their existing fork identity.
...
```bash
herdr pane report-agent <pane_id> --source custom:tool --agent codex --state working
herdr pane report-agent <pane_id> --source custom:tool --agent codex --state idle --session-id <id>
```

This fork does not install or uninstall agent hooks/plugins through `herdr integration`.
Use trusted local tools to report pane state through `pane report-agent` or the
`pane.report_agent` socket method.
...
new tab, a zoomed pane, or a workspace-scoped `popup` that does not change the tab
layout. Herdr keybindings can switch workspaces while the popup keeps running.
`--width` and `--height` set the outer popup dimensions in terminal
cells or percentages such as `80%`; omitted dimensions default to 80% of the
