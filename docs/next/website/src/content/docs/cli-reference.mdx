...
Use `--completion summary|full|none` to control the durable mailbox result sent to the exact caller after it becomes idle. Use `herdr job list`, `herdr job status <job>`, `herdr job log <job>`, and `herdr job cancel <job>` to manage jobs.
