...
- Ported upstream 0.9.1 `agent.prompt` and `herdr agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]`: reject empty, blocked, unknown, starting, and non-foreground targets before input; preserve bracketed paste and trailing newlines; acknowledge only after text, the submission delay, and Enter have been written. Waiting requires observed working/blocked activity and reports stalled prompts. Copilot focus handling and terminal-close failures follow upstream. The fork keeps `agent send`, durable mailbox delivery, and `p_N` / `pN` / `%N` pane targets. Launches through the fork's existing argv-based `agent start`, including shell, environment and language-runtime wrappers, now track upstream's pending/blocked/ready phases (3-second settling period, 30-second startup deadline) without replacing that CLI. Detection identifies wrapped agents; non-agent jobs retain their names. Snapshots omit pending managed names and restore active ownership. Agent release and name-only detection changes reach waiters, including shell respawn after process exit. Windows mailbox cancellation now prevents already-failed queued input from being written.
...
### Changed
- Bundled agent detection manifests can declare `fork_owned = true`; for those agents the fork's bundled rules are always used even when herdr.dev serves a newer upstream manifest. Devin is fork-owned so its fork-only working-state rules cannot be silently replaced.
