...
ローカルとリモートのプラットフォームが一致する場合、直接インストールでは Herdr は現在のローカルバイナリをコピーできます。この fork は `https://herdr.dev/latest.json` からリリースアセットをダウンロードしません。パッケージマネージャー由来のインストール、カスタムバイナリ、またはプラットフォーム不一致では、リモートアタッチ前に `HERDR_REMOTE_BINARY` へ明示的なローカルファイルパスを設定してください。
