...
Right-click a workspace to assign it to `⭐ favorites`, `💼 work`, or `🏠 personal`. Click a section header to collapse or expand that section. The agents panel only shows agents from expanded workspace sections.
...
Herdr is mouse-native. You can click panes, tabs, workspaces, and agents; drag borders; select text; and use right-click menus. Double-clicking a pane opens the same pane menu as right click.
