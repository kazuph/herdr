...
`pane current` prints the `HERDR_PANE_ID` for the calling pane after validating it with the running server. It fails instead of falling back to the focused pane when `HERDR_PANE_ID` is not set.
