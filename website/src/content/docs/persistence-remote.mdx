...
When your local and remote platforms match, Herdr can copy the current local binary. Automatic release downloads are disabled in this fork, so cross-platform remote attach requires an explicit local binary.
