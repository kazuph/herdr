...
kazuph/herdr fork 不通过上游安装器或预览通道分发 Windows 测试版二进制。需要时请从这个 source tree 构建并测试 Windows 支持。
...
Herdr 可以在正确的目录中启动窗格,并能从你启动 Herdr 的目录创建初始工作区。启动之后 PowerShell 的目录变化则不同: Herdr 能检查的进程字段无法可靠跟踪后续的逻辑 `cd` 变化。目录变化后请重新打开工作区,或使用支持 OSC7 的 shell 来报告实时 cwd。
